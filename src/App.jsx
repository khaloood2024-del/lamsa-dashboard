...
const CONFIG = {
  ...
  primaryColor: "#2563EB",
  services: [
    { id:1, name:"الخدمة الأولى",   duration:30,  price:100, icon:"⭐" },
    { id:2, name:"الخدمة الثانية",  duration:60,  price:200, icon:"💎" },
    { id:3, name:"الخدمة الثالثة",  duration:45,  price:150, icon:"🌟" },
    { id:4, name:"الخدمة الرابعة",  duration:90,  price:300, icon:"✨" },
    { id:5, name:"الخدمة الخامسة",  duration:60,  price:250, icon:"🎯" },
    { id:6, name:"الخدمة السادسة",  duration:120, price:400, icon:"💫" },
  ],
};
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  input,select,button,textarea { font-family:'IBM Plex Sans Arabic',sans-serif; }
`;
...
const Avatar = ({ name, role, size=36 }) => {
  const bg = role==="admin" ? T.purple : T.accent;
  return (
...
const focusIn  = e => { e.target.style.borderColor=T.accent; e.target.style.background="#fff"; e.target.style.boxShadow=`0 0 0 3px ${T.accent}18`; };
const focusOut = (err=false) => e => { e.target.style.borderColor=err?"#FECACA":T.border; e.target.style.background=T.bg; e.target.style.boxShadow="none"; };
      ...
      <div style={{ width:"45%", background:`linear-gradient(145deg,${T.sideBg} 0%,#1E3A5F 100%)`,
        ...
        <div style={{ position:"relative" }}>
          <div style={{ display:"inline-flex", alignItems:"center", justifyContent:"center",
            ...
            fontSize:24, marginBottom:32, boxShadow:`0 8px 24px ${T.accent}50` }}>
            🏢
          </div>
          <div style={{ color:"#fff", fontSize:28, fontWeight:700, marginBottom:8, lineHeight:1.2 }}>
            {CONFIG.businessName}
          </div>
        ...
        <div style={{ position:"relative" }}>
          {[
            { icon:"⚡", text:"إدارة المواعيد لحظة بلحظة" },
            { icon:"🤖", text:"استقبال ذكي على الواتساب" },
            { icon:"📊", text:"تقارير وإحصائيات تفصيلية" },
          ].map((f,i)=>(
            <div key={i} style={{ display:"flex", alignItems:"center", gap:12,
              marginBottom:i<2?16:0, opacity:0.8 }}>
              <div style={{ width:34, height:34, borderRadius:8,
                background:"rgba(255,255,255,0.08)",
                display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>{f.icon}</div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14,
            ...
            <Lbl label="اسم المستخدم">
              <input value={user} onChange={e=>{setUser(e.target.value);setError("");}}
                ...
                style={inp(!!error&&!user)} onFocus={focusIn} onBlur={focusOut()}/>
            </Lbl>
            ...
            <Lbl label="كلمة المرور">
              <div style={{ position:"relative" }}>
                <input type={showPwd?"text":"password"} value={pwd}
                  ...
                  style={{...inp(!!error&&user&&!pwd),paddingLeft:38}}
  ...
  return (
    <Modal title="🔒 صلاحية المدير" onClose={onClose} width={340}>
      <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:16}}>
        <Lbl label="كلمة مرور المدير">
          <div style={{position:"relative"}}>
            <input type={showPwd?"text":"password"} value={pwd}
              ...
              onKeyDown={e=>e.key==="Enter"&&handle()}
              placeholder="أدخل كلمة المرور"
              style={{...inp(error),paddingLeft:38}} autoFocus
              onFocus={focusIn} onBlur={focusOut(error)}/>
            <button onClick={()=>setShowPwd(!showPwd)} style={{
              position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",
        ...
        {error&&<div style={{color:T.red,fontSize:12}}>كلمة المرور غير صحيحة</div>}
      </div>
      <div style={{display:"flex",gap:8}}>
        <Btn onClick={handle} disabled={loading} style={{flex:1}}>{loading?"...":"دخول"}</Btn>
...
// ─── ADD BOOKING ─────────────────────────────────────────────────────
function AddBooking({ onClose, onAdd }) {
  const PRICES = Object.fromEntries(CONFIG.services.map(s=>[s.name,s.price+" ريال"]));
  const [form,  setForm]  = useState({name:"",service:CONFIG.services[0]?.name||"",date:"اليوم",time:"",phone:""});
  const [error, setError] = useState("");
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Lbl label="اسم العميل *">
            <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})}
              placeholder="الاسم الكامل" style={inp(!form.name&&!!error)}
              onFocus={focusIn} onBlur={focusOut()}/>
          </Lbl>
          <Lbl label="رقم الهاتف">
            <input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}
              placeholder="+966..." style={inp()} onFocus={focusIn} onBlur={focusOut()}/>
          </Lbl>
        </div>
        <Lbl label="الخدمة *">
          <select value={form.service} onChange={e=>setForm({...form,service:e.target.value})}
            style={{...inp(),cursor:"pointer"}} onFocus={focusIn} onBlur={focusOut()}>
            {CONFIG.services.map(s=><option key={s.name} value={s.name}>{s.icon} {s.name} — {s.price} ريال</option>)}
          </select>
        </Lbl>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Lbl label="التاريخ *">
          ...
          <Lbl label="الوقت *">
            <input value={form.time} onChange={e=>setForm({...form,time:e.target.value})}
              placeholder="مثال: 3:00 م" style={inp(!form.time&&!!error)}
              onFocus={focusIn} onBlur={focusOut()}/>
          </Lbl>
        </div>
...
function BookingRow({ b, onCancel }) {
  const sm = {
    confirmed:{label:"مؤكد",  type:"confirmed"},
  ...
  return (
    <div className="row-h" style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1.2fr 0.8fr 0.8fr auto",
      alignItems:"center",padding:"13px 20px",borderBottom:`1px solid ${T.border}`,background:T.surface}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <Avatar name={b.name} role="staff" size={32}/>
        <div>
          ...
          {phone&&<div style={{color:T.muted,fontSize:11,display:"flex",alignItems:"center",gap:3,marginTop:1}}>
            <Ico d={I.phone} size={10} color={T.muted}/>{phone}
          </div>}
        </div>
      </div>
      <div style={{color:T.textSoft,fontSize:13}}>{b.service}</div>
      <div style={{color:T.muted,fontSize:12,fontFamily:"'DM Mono',monospace"}}>{b.date} {b.time}</div>
      <div style={{color:T.accent,fontWeight:600,fontSize:13,fontFamily:"'DM Mono',monospace"}}>{b.price}</div>
...
function Reports({ bookings }) {
  ...
  const fail = bookings.filter(b=>b.status==="cancelled");
  ...
  const exportCSV = () => {
    ...
    const r=bookings.map(b=>[b.name,b.service,b.date,b.time,b.price,b.status==="confirmed"?"مؤكد":"ملغي"]);
  ...
  return (
    <div className="fu">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:20}}>
        <KPI label="إجمالي الحجوزات" value={bookings.length} sub="كل السجلات"       icon={I.cal}   color={T.accent} delay={0}/>
        ...
        <KPI label="إجمالي الإيرادات" value={rev.toLocaleString()+" ر"} sub="من المؤكدة" icon={I.trend} color={T.purple} delay={.15}/>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:16,marginBottom:16}}>
        {/* Services bar chart */}
        <div style={{background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:14,padding:"20px"}}>
          <div style={{color:T.text,fontWeight:600,fontSize:13,marginBottom:16,display:"flex",alignItems:"center",gap:6}}>
            <Ico d={I.trend} size={15} color={T.accent}/> الخدمات الأكثر طلباً
          </div>
          {sorted.length===0
              ...
              <div key={name} style={{marginBottom:14}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <span style={{color:T.purple,fontSize:11,fontWeight:500,fontFamily:"'DM Mono',monospace"}}>{s.revenue.toLocaleString()} ر</span>
                  </div>
                </div>
        ...
        <div style={{background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:14,padding:"20px"}}>
          <div style={{color:T.text,fontWeight:600,fontSize:13,marginBottom:16,display:"flex",alignItems:"center",gap:6}}>
            <Ico d={I.trend} size={15} color={T.accent}/> نسب الأداء
          </div>
          {[
            {label:"معدل الإكمال",value:bookings.length?Math.round((done.length/bookings.length)*100):0,color:T.green},
            {label:"معدل الإلغاء",value:bookings.length?Math.round((fail.length/bookings.length)*100):0,color:T.red},
          ].map(item=>(
          ...
          <div style={{paddingTop:14,borderTop:`1.5px solid ${T.border}`}}>
            {[
              ...
              {l:"متوسط قيمة الحجز",   v:(done.length?Math.round(rev/done.length):0).toLocaleString()+" ر", c:T.accent},
              {l:"إجمالي الإيرادات",    v:rev.toLocaleString()+" ر",                                   c:T.text},
            ].map(item=>(
              <div key={item.l} style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                ...
                <span style={{color:item.c,fontWeight:600,fontSize:13,fontFamily:"'DM Mono',monospace"}}>{item.v}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:14,overflow:"hidden"}}>
        <div style={{padding:"14px 20px",borderBottom:`1.5px solid ${T.border}`,
          ...
          <span style={{color:T.text,fontWeight:600,fontSize:13}}>سجل الحجوزات التفصيلي</span>
          <span style={{color:T.muted,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{bookings.length} سجل</span>
        </div>
        ...
        <div style={{maxHeight:280,overflow:"auto"}}>
          {bookings.length===0
            ...
            :bookings.map(b=>(
              <div key={b.id} className="row-h" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",
...
function UsersPage() {
  ...
  const fetch_ = async()=>{ try{const r=await fetch(API_URL+"/api/users");setUsers(await r.json());}catch{}finally{setLoading(false);} };
  useEffect(()=>{fetch_();},[]);
  ...
  const save = async()=>{
    ...
    try{
      ...
      await fetch_(); setShowForm(false);
    }catch{setFormErr("تعذر الاتصال");}finally{setSaving(false);}
  };

  const del = async(u)=>{ if(!window.confirm("حذف "+u.username+"؟")) return; await fetch(API_URL+"/api/users/"+u.id,{method:"DELETE"}); fetch_(); };
          ...
          <div style={{display:"flex",flexDirection:"column",gap:13,marginBottom:16}}>
            <Lbl label="اسم المستخدم *">
              <input value={form.username} onChange={e=>setForm({...form,username:e.target.value})}
                placeholder="اسم المستخدم" style={inp(!!formErr&&!form.username)} onFocus={focusIn} onBlur={focusOut()}/>
            </Lbl>
            <Lbl label={editUser?"كلمة مرور جديدة (اختياري)":"كلمة المرور *"}>
              <div style={{position:"relative"}}>
                <input type={showPwd?"text":"password"} value={form.password}
                  ...
                  style={{...inp(),paddingLeft:38}} onFocus={focusIn} onBlur={focusOut()}/>
...
function Dashboard({ onLogout, role, username }) {
  ...
  useEffect(()=>{
    ...
    es.onmessage=e=>{try{const d=JSON.parse(e.data);if(d.type==="new_booking"){fetchB();notif("حجز جديد — "+d.name,"success");}}catch{}};
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'IBM Plex Sans Arabic',sans-serif",direction:"rtl",display:"flex"}}>
      ...
      {showAdd&&<AddBooking onClose={()=>setShowAdd(false)} onAdd={addBooking}/>}
            ...
            <button key={t.id} className="nav-h t" onClick={()=>{
              ...
              {t.id==="bookings"&&pending.length>0&&(
                <span style={{marginRight:"auto",background:`${T.accent}30`,color:T.accent,
                  ...
                  fontFamily:"'DM Mono',monospace"}}>{pending.length}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <div style={{marginRight:220,flex:1,padding:"24px 28px",minHeight:"100vh"}}>
        ...
        {tab==="overview"&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:22}}>
              ...
              <KPI label="ملغية"          value={cancelled.length} sub="إجمالي الإلغاءات"          icon={I.warn}  color={T.red}    delay={.15}/>
            </div>

            <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:16}}>
              {/* Today */}
              <div className="fu" style={{background:T.surface,border:`1.5px solid ${T.border}`,
                borderRadius:14,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",animationDelay:"0.2s"}}>
                ...
                {today.length===0
                  ...
                  :today.map(b=>(
                    <div key={b.id} className="row-h" style={{display:"flex",alignItems:"center",
                      gap:10,padding:"12px 18px",borderBottom:`1px solid ${T.border}80`,background:T.surface}}>
                      <Avatar name={b.name} role="staff" size={32}/>
                      <div style={{flex:1}}>
                        <div style={{color:T.text,fontSize:13,fontWeight:500}}>{b.name}</div>
              ...
              {/* Top services */}
              <div className="fu" style={{background:T.surface,border:`1.5px solid ${T.border}`,
                borderRadius:14,padding:"16px 18px",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",animationDelay:"0.25s"}}>
                ...
                {CONFIG.services.slice(0,5).map(s=>{
                  ...
                  const mx=Math.max(...CONFIG.services.map(sv=>bookings.filter(b=>b.service===sv.name).length),1);
                  return(
          ...
          <div className="fu">
            <div style={{display:"flex",gap:10,marginBottom:16,alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,flex:1,
                background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:10,padding:"8px 12px"}}>
                <Ico d={I.search} size={14} color={T.muted}/>
              ...
              <div style={{display:"flex",gap:3,background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:10,padding:3}}>
                {[{id:"all",l:"الكل"},{id:"confirmed",l:"مؤكدة"},{id:"pending",l:"معلقة"},{id:"cancelled",l:"ملغية"}].map(f=>(
                  <button key={f.id} onClick={()=>setFilter(f.id)} className="t" style={{
                    ...
                    {f.l}
                    {f.id!=="all"&&<span style={{opacity:.7,fontSize:10,marginRight:3,fontFamily:"'DM Mono',monospace"}}>
            ...
            <div style={{background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:14,overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1.2fr 0.8fr 0.8fr auto",
                padding:"10px 20px",background:T.bg,borderBottom:`1.5px solid ${T.border}`}}>
                {["العميل","الخدمة","الموعد","السعر","الحالة",""].map(h=>(
                  <div key={h} style={{color:T.muted,fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5}}>{h}</div>
                ))}
              </div>
              {filtered.length===0
                ...
                :filtered.map(b=><BookingRow key={b.id} b={b} onCancel={cancelBooking}/>)
              }
            </div>
          </div>
        )}
        ...
        {/* SERVICES */}
        {tab==="services"&&(
          <div className="fu" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
            {CONFIG.services.map((s,i)=>{
              ...
              return(
                <div key={s.id} className="hover-lift fu t" style={{
                  ...
                  <div style={{color:T.muted,fontSize:12,marginBottom:14,display:"flex",alignItems:"center",gap:4}}>
                    <Ico d={I.clock} size={12} color={T.muted}/>{s.duration} دقيقة
                  </div>
                  <div style={{paddingTop:12,borderTop:`1.5px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{color:T.accent,fontWeight:700,fontSize:16,fontFamily:"'DM Mono',monospace"}}>{s.price} ر</span>
                    <span style={{color:T.muted,fontSize:11}}>{s.price*cnt>0?`إيراد: ${(s.price*cnt).toLocaleString()} ر`:""}</span>
                  </div>
                </div>
              );
            })}
